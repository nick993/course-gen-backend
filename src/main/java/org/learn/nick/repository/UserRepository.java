...
public class UserRepository {
    ...
    public List<User> allUsers() {
        return StreamSupport.stream(users.find().spliterator(), true)
                .map(UserRepository::user)
    ...
    public User findByName(String name) {
        Document doc = users.find(eq("name", name)).first();
        return user(doc);
    }
    ...
    private static User user(Document doc) {
        if (doc == null) {
            return null;
        }
        return new User(
